...
Built handTrackingModule.py, a handDetector class that wraps MediaPipe's solutions.hands API, with findHands() to detect and draw the hand skeleton and findPosition() to pull out the 21 landmark points as pixel coordinates.
...
Ended up with 6 gestures that got classified reliably: FIST, OPEN_PALM, POINT, PEACE, THUMB, PINCH.
...
Final gesture set for the game: index finger up for up, thumb out for left, pinky out for right, closed fist for down, and open palm to pause.
...
Handedness and mirror bug. After flipping the frame with cv2.flip() to get a natural mirror view, MediaPipe's handedness label is still based on the unflipped frame, so it reads backwards if you flip before detecting but then also try to correct the label afterward. Fixed by being consistent, flip first, then detect, and not correcting the label a second time.
...
Typo in the model file path (a comma instead of a period) which silently broke the HandLandmarker setup with a confusing error.
Corrupted model file. Downloaded the hand_landmarker.task file through the browser once and it saved an error page instead of the actual model. Since .task files are zip archives internally, this threw an unable to open zip archive error. Fixed by downloading it directly and checking the file size and that it actually opens as a zip.
Angle calculated after it was used. In an earlier version of the gesture logic I was computing the pointing angle after the if/else block that used it, so the angle value was always stale. This is part of why I moved away from angle based detection toward the finger up/down pose approach for the game.
